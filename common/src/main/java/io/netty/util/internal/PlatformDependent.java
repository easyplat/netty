...
public final class PlatformDependent {
    ...
    public static ByteBuffer allocateDirectNoCleaner(int capacity) {
        if (DIRECT_MEMORY_COUNTER != null) {
            for (;;) {
                long used = DIRECT_MEMORY_COUNTER.get();
                long expect = used + capacity;
                if (expect > maxDirectMemory()) {
                    throw new OutOfMemoryError("No more memory available");
                }
                if (DIRECT_MEMORY_COUNTER.compareAndSet(used, expect)) {
                    break;
                }
            }
        }
        try {
            ...
            // Unsafe.allocateMemory() threw a OutOfMemoryError, decrement the counter and rethrow.
            if (DIRECT_MEMORY_COUNTER != null) {
    ...
    private static boolean useDirectBufferNoCleaner0() {
        return hasUnsafe() && !SystemPropertyUtil.getBoolean("io.netty.noDirectBufferNoCleaner", true)
